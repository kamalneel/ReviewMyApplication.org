    ...
    return {
      ...
      select(columns = '*') {  // NOT async - returns object for chaining
        ...
        return {
          ...
          order(column, options = {}) {  // NOT async - returns object for chaining
            const direction = options.ascending === false ? 'desc' : 'asc';
            queryParams.set('order', `${column}.${direction}`);
            return this;
          },
          async single() {  // async - makes the actual network call
            const response = await fetch(
              `${SUPABASE_URL}/rest/v1/${table}?${queryParams.toString()}`,
              {
                headers: {
                  'apikey': SUPABASE_ANON_KEY,
                  'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                  'Accept': 'application/vnd.pgsql.object+json'
                }
              }
            );
            const result = await response.json();
          ...
          async execute() {  // async - fetch multiple rows
            const response = await fetch(
              `${SUPABASE_URL}/rest/v1/${table}?${queryParams.toString()}`,
              {
                headers: {
...
// ═══════════════════════════════════════════════════════════════
// SCHOOL DATA - Synthetic Admissions Officers
// ═══════════════════════════════════════════════════════════════
...
const SCHOOLS = [
  {
    ...
    description: 'Stanford seeks students who demonstrate intellectual vitality, entrepreneurial thinking, and a passion for making a difference.',
  ...
  },
  {
    ...
    us_news_rank: 2,
    tagline: 'Hands-on makers (Mens et Manus)',
    description: 'MIT values students who build, create, and solve problems with their hands as much as their minds.',
    colors: { primary: '#A31F34', secondary: '#8A8B8C' },
    num_supplemental_essays: 6
  },
  {
    code: 'harvard',
    ...
    location: 'Cambridge, MA',
    us_news_rank: 1,
    tagline: 'World-class leadership and distinction',
    description: 'Harvard seeks students who have demonstrated excellence and leadership potential across academics and extracurriculars.',
    colors: { primary: '#A51C30', secondary: '#FFFFFF' },
    num_supplemental_essays: 5
  }
];

// ═══════════════════════════════════════════════════════════════
// HELPER FUNCTIONS FOR SYNTHETIC ADMISSIONS SYSTEM
// ═══════════════════════════════════════════════════════════════

/**
 * Generate unique application ID
 * Format: RMA-YYYY-XXX-AAAA
 * Example: RMA-2026-STF-7F3A (Stanford), RMA-2026-MIT-K9L2 (MIT)
 */
function generateApplicationID(schoolCode) {
  const year = new Date().getFullYear();
  const schoolPrefix = schoolCode.substring(0, 3).toUpperCase();
  const randomSuffix = generateRandomString(4);

  return `RMA-${year}-${schoolPrefix}-${randomSuffix}`;
}
...
/**
 * Hash identifier (email or session ID) for student tracking
 * Simple hash function for version tracking across submissions
 */
...
let appState = {
  currentProgram: null,
  selectedSchool: null,        // NEW: Selected school code ('stanford', 'mit', 'harvard')
  currentStep: 0,
  formData: {},
  uploadedFiles: {},
  applicationID: null           // NEW: Generated application ID
};
...
function showLandingPage() {
  ...
  document.getElementById('college-selection-page').classList.add('hidden');
  ...
  appState.applicationID = null;
  window.scrollTo(0, 0);
}

// ═══════════════════════════════════════════════════════════════
// COLLEGE SELECTION PAGE FUNCTIONS
// ═══════════════════════════════════════════════════════════════

// Show college selection page
function showCollegeSelection() {
  document.getElementById('landing-page').classList.add('hidden');
  document.getElementById('college-selection-page').classList.remove('hidden');
  document.getElementById('application-page').classList.add('hidden');
  document.getElementById('results-page').classList.add('hidden');
  document.getElementById('demo-page').classList.add('hidden');
...
// Render school cards in grid
function renderSchoolGrid() {
  const grid = document.getElementById('schools-grid');

  grid.innerHTML = SCHOOLS.map(school => `
    <div class="school-card" data-tier="${school.tier}">
      <div class="school-header" style="background: linear-gradient(135deg, ${school.colors.primary} 0%, ${school.colors.primary}dd 100%);">
        <div class="school-rank">#${school.us_news_rank} US News</div>
      ...
      <div class="school-body">
        <div class="school-stats">
          <div class="stat">
            ...
            <span class="stat-value">${school.acceptance_rate}</span>
          </div>
          <div class="stat">
            <span class="stat-label">Supplementals</span>
            <span class="stat-value">${school.num_supplemental_essays} essays</span>
          </div>
        </div>
        ...
        <button class="btn btn-primary btn-block" onclick="selectSchool('${school.code}')">
          Apply to Synthetic ${school.shortName}
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 18px; height: 18px; margin-left: 0.5rem;">
...
// Filter schools by search and tier
function filterSchools() {
  ...
  schoolCards.forEach(card => {
    ...
    const schoolTier = card.getAttribute('data-tier');

    const matchesSearch = schoolName.includes(searchTerm);
    const matchesTier = tierFilter === 'all' || schoolTier === tierFilter;
...
// Select a school and start application
async function selectSchool(schoolCode) {
  appState.selectedSchool = schoolCode;
  appState.currentProgram = 'college';

  const school = SCHOOLS.find(s => s.code === schoolCode);
  console.log(`Selected: ${school.name}`);

  // TODO: Load school-specific form (next phase)
  // For now, just show a loading message
  alert(`Loading application for ${school.name}...\n\nThis will load school-specific supplemental essays.\n\n(Dynamic form builder coming in next phase!)`);
}
